...
void ObstacleDistanceGrid::setDistances(const OccupancyGrid& map)
{
    ...
    //printf("we are trying to intializing the distances\n");
    initializeDistances(map);
    //printf("the distances are initialized\n");

    std::priority_queue<DistanceNode> searchQueue;
    enqueue_obstacle_cells(map, *this, searchQueue);

    // Edge case: If there are no walls, we set obstacle distances to be arbitrarily high
    if (searchQueue.empty() && is_cell_free(cell_t(0,0), map)){
        //printf("Edge case\n");
        //printf("%d",map.logOdds(10, 10));
        //printf("we are initializing the map\n");
        int width = map.widthInCells();
        //printf("we are getting the width information\n");
        int height = map.heightInCells();
        ...
        for (cell.y = 0; cell.y < height; ++cell.y)
        {
            if (cell.y % 100 == 0)
            {
                //printf("Cell.y = %d", cell.y);
                //printf("a");
            }
            for (cell.x = 0; cell.x < width; ++cell.x)
            {
                distance(cell.x,cell.y) = 10000.0;
            }
            
        }
        //printf("Done fixing edge case");
    }
