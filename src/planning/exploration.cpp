...
{
    std::cout << "Started Explore Environment" << std::endl;
...
int8_t Exploration::executeExploringMap(bool initialize)
{
    ...
    */
    ...
    if(front_processing.path_selected.path_length > 0){
        currentPath_ = front_processing.path_selected;
        std::cout << "[Path Success], Frontier(" << currentPath_.path.end()->x << "," << currentPath_.path.end()->y << ")" << std::endl;
    }
    ...
    std::cout << "NUM of unreachable frontiers: " << front_processing.num_unreachable_frontiers << std::endl;

    // No reachable frontiers, means exploration is complete
    if (front_processing.num_unreachable_frontiers == frontiers_.size())
    {
        status.status = mbot_lcm_msgs::exploration_status_t::STATUS_COMPLETE;
    }

    // If there is a path, then we're in progress
    else if(front_processing.path_selected.path_length > 1)
    {
        status.status = mbot_lcm_msgs::exploration_status_t::STATUS_IN_PROGRESS;
    }
