...
private:
    float pid[3] = {1.5, 2.5, 0.1}; //kd, ka, kb
...
int main(int argc, char** argv)
{
    ...
    while(true)
    {
    	...
    	if(controller.timesync_initialized()){
            ...
            // Fwd vel
            float vx_desired = 0.2;
            // float vx_desired = 0.8;
            if (cmd.vx > vx_desired) cmd.vx = vx_desired;
            else if (cmd.vx < -vx_desired) cmd.vx = -vx_desired;
