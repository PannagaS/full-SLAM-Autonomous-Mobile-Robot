...
, initialPose_(initialPose)
{
    ...
    switch (mode_) {
        case mapping_only:
            mode_str = " [mapping_only]";
            break;
        case localization_only:
            mode_str = " [localization_only]";
            break;
        case action_only:
            mode_str = " [action_only]";
            break;
        default:
            mode_str = " [full_slam]";
            break;
    }
    std::cout << LOG_HEADER << "SLAM initialized in mode " << mode_ << mode_str << " with "<< numParticles << " particles" << std::endl;
}
...
void OccupancyGridSLAM::updateLocalization(void)
{
    if(haveMap_ && (mode_ != mapping_only))
    {
        ...
        mbot_lcm_msgs::particles_t particles = filter_.particles();
